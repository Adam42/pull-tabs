...
- [x] **MEDIUM / L — Build toolchain has 48 known vulns (3 critical: pbkdf2,
  sha.js, shell-quote)** — all in the laravel-mix/webpack dev chain. Done
  (Phase 6, 0.20.0): the build was migrated off laravel-mix to esbuild via a
  small `build.js`; the entire webpack/laravel-mix dependency chain was
  removed, eliminating the vulns wholesale.
...
- [x] **HIGH impact / L — Replace laravel-mix with a modern bundler.** Done
  (Phase 6, 0.20.0): migrated to esbuild via `build.js`; `webpack.mix.js` and
  `mix-manifest.json` deleted, laravel-mix/webpack deps removed. Manifests are
  now valid standalone JSON shallow-merged at build with the version injected
  from `package.json`; `npm run dev`/`watch` reimplemented on esbuild.
- [x] **MEDIUM impact / S — Modernize clipboard copy** from deprecated
  `document.execCommand("Copy")` + hidden textarea to
  `navigator.clipboard.writeText()`. Done (Phase 7.3, 0.20.0): `writeText` is
  the primary path with the hidden-textarea/`execCommand` retained only as a
  `catch` fallback (no new permission, no Firefox regression).
...
- [x] **MEDIUM impact / M — Consolidate `localStorage` →
  `browser.storage.local`.** Done (Phase 7.1, 0.20.0): both `pullTabsFolderId`
  and `initialSetup` moved to `browser.storage.local` via the `storage.js`
  wrapper; key constants extracted to `src/js/storageKeys.js`;
  `browserUtils.init()` runs a one-time per-key migration for existing users
  and is awaited before `popup.init()`.
- [x] **LOW impact / M — Background script for download tracking.** Done
  (Phase 7.2, 0.20.0): an MV3 background script (`src/js/background.js`) owns
  the `downloads.onChanged` listener, so completion + autoclose survive the
  popup closing. The popup subscribes via `downloadStatus.js`; a synchronous
  in-flight claim + `reconcile-download` message guarantee exactly one
  autoclose/broadcast per download (incl. the fast-completion race).
...
- [x] **S — Single-source the version number.** Done (Phase 6, 0.20.0):
  `version` removed from `src/manifest-base.json`; `build.js` injects
  `package.json`'s version into both dist manifests. One place to bump.
...
3. ~~**2017 prototype files**~~ — **RESOLVED 2026-07-05: deleted** (staged
   in the WIP landing).
4. ~~**Build tool swap**~~ — **RESOLVED 2026-07-10: signed off.** laravel-mix
   replaced with esbuild (`build.js`) in Phase 6 / 0.20.0; README contributor
   commands updated.
